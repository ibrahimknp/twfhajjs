...
public class V_Luk_Area_ServiceImpl extends RemoteServiceServlet implements V_Luk_Area_Service{
	
	private static final long	serialVersionUID	= -8480133366196663439L;
	ExceptionsContainer excep=ExceptionsContainer.getInstance();
	private static Logger log = Logger.getLogger( V_Luk_Area_ServiceImpl.class );
	Date dt = new Date();
	...
	public String save(V_Luk_Area_Wrapper meta)throws MyExceptions
	{
		...
		try 
		{
			...
			session.save(meta.getV_Luk_Area());
			session.getTransaction().commit();
			
			return (meta.getV_Luk_Area().getId()).toString();
		} 
		catch (Throwable thr) 
		{
			log.error("save",thr);
	...
	public String update(V_Luk_Area_Wrapper meta)throws MyExceptions
	{
		...
		try 
		{
			...
			session.update(meta.getV_Luk_Area());
			session.getTransaction().commit();
	...
	public V_Luk_Area_Wrapper update_return(V_Luk_Area_Wrapper meta)throws MyExceptions
	{
		...
		try 
		{
			...
			if(meta.getQuery()!=null)
			{
				...
				V_Luk_Area_Wrapper mm = new V_Luk_Area_Wrapper();
				mm.setQuery(meta.getReturn_query());
				meta = getListData(mm);
				V_Luk_Area hv =(V_Luk_Area)meta.getListV_Luk_Area().get(0);
				meta.setV_Luk_Area(hv);
			}
			else
			{
				session.update(meta.getV_Luk_Area());
	...
	public V_Luk_Area_Wrapper deleteSave(V_Luk_Area_Wrapper meta)throws MyExceptions
	{
		...
		try 
		{
			...
			qry.executeUpdate();
			List<V_Luk_Area> lst=meta.getListV_Luk_Area();
			for(V_Luk_Area s : lst)
			{
				session.save(s); 
			}
			session.getTransaction().commit();
